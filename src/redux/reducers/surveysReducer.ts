...
export const surveysReducer = (
  ...
  switch (action.type) {
    ...
    case types.surveyUpdated:
      return {
        ...state,
        surveys: state.surveys.map((survey: Partial<Survey>) =>
          survey.idSurvey === action.payload.idSurvey ? action.payload : survey
        ),
      };
    ...
    case types.surveyDelete:
      return {
        ...state,
        surveys: state.surveys.filter(
          (survey) => survey.idSurvey !== action.payload
        ),
      };
