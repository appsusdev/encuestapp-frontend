...
export const AssignSurvey = () => {
  ...
  const { successAlert, errorAlert } = useSelector<AppState, AppState["ui"]>(
    (state) => state.ui
  );
  ...
  const surveysAssign = list.filter(
    (survey: Survey) => surveyor && survey.surveyors.includes(surveyor.email)
  );

  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, surveysAssign.length - page * rowsPerPage);
  ...
  const handleAssign = async () => {
    ...
    setAssign(true);
    survey.trim() &&
      (await dispatch(startAssignSurvey(survey, surveyor.email, action)));
    setLoading(false);
  };
  ...
  return (
    <Box m={1}>
      ...
      <MyAlert
        open={successAlert}
        typeAlert="success"
        message={assign ? "AssignedSurvey" : "SurveyDeleted"}
        time={2000}
