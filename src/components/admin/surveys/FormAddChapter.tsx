...
export const FormAddChapter = () => {
  ...
  const { activeSurvey, activeChapter } = useSelector<
    AppState,
    AppState["survey"]
  >((state) => state.survey);
  const { errorAlert, successAlert, modalAlert } = useSelector<
    AppState,
    AppState["ui"]
  >((state) => state.ui);
      ...
      <Formik
        ...
        onSubmit={async (data, { setSubmitting, resetForm }) => {
          ...
          if (chapter) {
            action = false;
            await dispatch(
              startNewChapter(data, survey.idSurvey, action, chapter.id)
            );
          } else {
              ...
              {chapter && (
                <Grid container spacing={2}>
                  ...
                  >
                    <Grid item xs={8}>
                      {" "}
                      <FormattedMessage id="EditChapterMessage" />
                    </Grid>
                    <Grid item xs={3}>
                      <Button
